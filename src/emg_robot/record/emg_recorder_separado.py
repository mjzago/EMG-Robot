...
while True:
    ...
    acc_raw = get_imu_acc_raw(IMU)
    gyro_raw = get_imu_gyro_raw(IMU)

    # Separate printing and writing for each column
    for i in range(len(values)):
        print(f"Data {i+1}: {values[i]}")
        values_row = values[:i] + [None] + values[i+1:] + acc_raw + gyro_raw + [delta]
